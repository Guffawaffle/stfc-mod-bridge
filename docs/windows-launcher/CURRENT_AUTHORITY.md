...
STFC Mod Bridge remains pre-release. `v0.1.0-rc.4` is the approved public canary
entry point. Later candidates are qualification artifacts unless their release
notes explicitly approve public installation. The draft rc.15 App Installer
asset is not the final candidate; issue #30 owns the next immutable signed build
and its clean-install, update, dogfood, accessibility, and evidence decision.
