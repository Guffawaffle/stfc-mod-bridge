# Standalone launcher release manifest

Tagged releases in `Guffawaffle/stfc-mod-launcher` publish
`stfc-mod-launcher-release-manifest.json`. This repository is the only launcher
self-update authority. Distribution provider packs describe mod artifacts and
cannot supply or redirect the launcher repository, manifest name, publisher,
or update channel.
...
| ID | Role | User-facing? |
|---|---|---|
| `windows-launcher-setup-x64` | Signed per-user setup executable | Yes; the only install asset |
| `windows-launcher-archive-x64` | ZIP containing the signed launcher and replace-on-exit helper | No; machine-consumed self-update input |
...
`STFCCommunityMod.Launcher.Setup.exe`, that every required input is a PE, and
...
The launcher uses `GitHubLauncherReleaseClient`, which is separate from the
...
schema, and selects exactly one supported launcher archive. A launcher-only
...
Both extracted PEs must pass Authenticode for the launcher-owned publisher, and
the signed launcher's embedded source revision must match the manifest's tagged
commit.

## Replay and withdrawal policy

Stable and preview are distinct; the normal launcher requests stable. A
candidate must numerically advance the running launcher version. Equal or lower
