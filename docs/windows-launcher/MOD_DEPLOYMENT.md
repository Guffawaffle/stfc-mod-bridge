...
| Verified managed DLL | Same provider/channel as preference | Installed provider, channel, and version | **Update mod**; the action checks the selected source before offering installation |
...
| **Update mod** (release check) | Resolved preferred provider/channel and safe local evidence capture. | The visible Home action is the explicit network action. Discovery itself performs no mutation; when a newer trusted release is found, show the source, release, exact game target, and one installation confirmation. | The release check may run while STFC runs, but installation remains disabled until it closes. No restart. | Store only bounded, expiring observation metadata keyed to preference and current artifact evidence. Failure leaves local runnable/managed state truthful and offline-capable. |

For v1, the Home **Update mod** action combines the explicit latest-release check and review into one guided flow. It
must still label the network step and receive affirmative input before installation; opening the source selector or
displaying Home never triggers discovery. Mod Bridge application updates remain a separately named trust domain.
