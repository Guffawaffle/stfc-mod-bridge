        ...
        IReadOnlyList<LauncherConfigurationSetting> settings)
    {
                     ...
                     candidate => candidate.Control == LauncherConfigurationControl.NotificationPolicy))
        {
            ...
            var hasCompleteProviderMetadata =
                hasExpandedDefault
                && invalidValueBehavior != "unknown"
                && setting.Provenance is not ({ RuntimePath: "unknown" } or { DefaultSource: "unknown" });
            ...
            {
                if (!string.Equals(alias.Status, "deprecated", StringComparison.Ordinal)
                    || !string.Equals(
                        alias.Precedence,
                        "canonical-replaces-whole-policy",
                        StringComparison.Ordinal)
                    || string.IsNullOrWhiteSpace(alias.Removal))
                {
                    throw new LauncherConfigurationSchemaException(
